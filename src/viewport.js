/** Конфигурирование и инициализация холста и области просмотра WebGL2.
 * @param {object} options - Объект с параметрами
 * @param {number} [options.width] - Ширина в пикселях
 * @param {number} [options.height] - Высота в пикселях
 * @param {number} [options.x=0] - Горизонтальное смещение
 * @param {number} [options.y=0] - Вертикальное смещение
 * @param {string} [options.background="#FFFFFF"] - Цвет фона
 * @param {boolean} [options.debug=false] - Выводить отладочную информацию
 * @returns {WebGL2RenderingContext} WebGL контекст
 */
export default ({ width, height, x, y, background, debug = false }) => {
  const canvas = getCanvas()
  const gl = getContext({ canvas, debug })
  setSize({ gl, width, height, x, y, debug })
  setBackgroundColor({ gl, hex: background, debug })
  return gl
}
 ...
 * @param {string} [options.hex="#FFFFFF"] - Цвет в шестнадцатеричном формате, ожидается строка формата '#FFFFFF'.
 * @param {boolean} [options.debug=false] - Выводить отладочную информацию
 */
export const setBackgroundColor = ({ gl, hex = "#FFFFFF", debug = false }) => {
